...
type EncryptedResponse = { id: string; type: "encrypted"; packageBytes: ArrayBuffer; checksum: string; durationMs: number }
type DecryptedResponse = { id: string; type: "decrypted"; recoveredBytes: ArrayBuffer; metadata: CipherPixMetadata; durationMs: number }
type WorkerResponse = StageResponse | ErrorResponse | EncryptedResponse | DecryptedResponse
...
function runWorker<T extends EncryptedResponse | DecryptedResponse>(
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onerror = () => {
      worker.terminate()
      reject(new Error("This browser could not start the local encryption worker."))
    }
...
export function encryptInWorker(
  ...
  key: number,
  rails: number,
  file: { name: string; mimeType: string; extension: string; width: number; height: number },
  onStage: (stage: ProcessingStage) => void,
): Promise<EncryptedResponse> {
  const transferable = Uint8Array.from(bytes).buffer
  return runWorker({ id: crypto.randomUUID(), type: "encrypt", bytes: transferable, key, rails, file }, [transferable], onStage)
}
...
export function decryptInWorker(
  ...
  return runWorker({ id: crypto.randomUUID(), type: "decrypt", packageBytes: transferable, key, rails }, [transferable], onStage)
}
